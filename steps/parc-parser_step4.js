...
collect(parse_rulearrow(), result)
				...
				function parse_rulearrow() {
					...
					return [{type: "rulearrow", value: result}];
...
collect(parse_cond(), result)
				...
				function parse_nots() {
					let result = [];
						...
						if (collect(parse_not(), result)) {
							while(collect(parse_not(), result)) {};
...
while (parseGroup0()) {};
				...
				function parse_lb() {
					...
					return [{type: "lb", value: joinTokens(result)}];
